...
export function useChatRoom(ticketId: string) {
  ...
  useEffect(() => {
    ...
    function onTyping(payload: { userId: string; ticketId: string }) {
      ...
      typingTimeoutRef.current = setTimeout(() => setOperatorTyping(false), TYPING_TIMEOUT_MS);
    }
    ...
    // The gateway's handleConnection only rejoins userRoom on (re)connect,
    // not ticketRoom — that only happens via this ticket:join emit. Without
    // re-emitting it here too, a socket.io auto-reconnect after a network
    // blip leaves the client silently out of the ticket's room: no more
    // messages/edits/typing until they navigate away and back. 'connect'
    // fires on every successful (re)connection, not just the first — no
    // double-join risk for the current connection, since a listener attached
    // to an already-connected socket doesn't fire retroactively (the
    // explicit emit below covers that case instead).
    function onConnect() {
  ...
  const sendMessage = useCallback(
    (body: string): Promise<PublicComment> => {
      return new Promise((resolve, reject) => {
        getChatSocket()
          .timeout(10000)
          .emit('ticket:message', { ticketId, body }, (err: Error | null, response: PublicComment | { error: true; message: string }) => {
            if (err) reject(err);
  ...
  // Mirrors sendMessage's ack-based flow above (and operator-app's own
  // editMessage) — the gateway's handleEditMessage acks the same way
  // handleMessage does (the edited comment on success, `{ error: true,
  // message }` on a server-side rejection: ticket closed/trashed in the
  // meantime, or the edit window on this comment already expired). This
  // used to be pure fire-and-forget with no ack at all, so a rejected edit
  // still cleared the composer and closed the editing UI as if it had
  // gone through, leaving the original text untouched in the thread with
  ...
  return { messages, operatorTyping, viewerIds, sendMessage, editMessage, notifyTyping };
}
