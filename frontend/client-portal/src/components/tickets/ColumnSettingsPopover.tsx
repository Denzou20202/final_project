...
// Reorder-only version of operator-app's ColumnSettingsPopover (see that
// copy's own comments) — no hide/resize here, just up/down, since that's
// all the client-facing list needs.
//
...
export function ColumnSettingsPopover({
  ...
  return createPortal(
    <div
      ...
      className="fixed z-40 w-56 rounded-xl border border-border bg-surface-card p-2 shadow-lg"
    >
      ...
      {order.map((key, index) => {
        ...
        return (
          <div key={key} className="flex items-center gap-2 rounded-lg px-2 py-1 hover:bg-surface-muted">
            <label className="min-w-0 flex-1 truncate text-[13px]">{label}</label>
            <button
              type="button"
              onClick={() => move(key, -1)}
              disabled={index === 0}
