...
interface RecentActivityState {
  ...
  // store's sibling, sidebar-highlight.store.ts: without this, the next
  // client to log in on this browser inherits the previous client's "recent
  // activity" dots, since logging out never used to clear it. Call on logout.
  clearAll: () => void;
}
...
export const useRecentActivityStore = create<RecentActivityState>()(
  persist(
    ...
    { name: 'veloxdesk-client-recent-activity' },
  ),
);
