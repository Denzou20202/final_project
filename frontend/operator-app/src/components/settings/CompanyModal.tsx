...
import { useCreateCompany, useRenameCompany } from '../../hooks/useCompanies.js';
import { getErrorMessage } from '../../lib/errors.js';
import type { PublicCompany } from '../../lib/types.js';

export function CompanyModal({ existing, onClose }: { existing: PublicCompany | undefined; onClose: () => void }) {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (!trimmed) return;
    if (existing) {
      renameCompany.mutate({ id: existing.id, name: trimmed }, { onSuccess: onClose });
    } else {
      createCompany.mutate(trimmed, { onSuccess: onClose });
    }
  }

  const mutation = existing ? renameCompany : createCompany;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 sm:px-4">
      <div className="flex h-full w-full flex-col overflow-y-auto bg-surface-card p-6 shadow-lg sm:h-auto sm:max-h-[90vh] sm:max-w-md sm:rounded-2xl sm:border sm:border-border">
        <h2 className="mb-4 font-display text-base font-bold">
          {existing ? t('admin.companies.editTitle') : t('admin.companies.newTitle')}
        </h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-3" noValidate>
          <div>
            <label htmlFor="company-name" className="mb-1 block text-sm font-medium text-ink-muted">
              {t('admin.companies.nameLabel')}
            </label>
            <input
              id="company-name"
              value={name}
            ...
            />
            {!name.trim() && <p className="mt-1 text-xs text-ink-faint">{t('admin.companies.nameRequired')}</p>}
          </div>
