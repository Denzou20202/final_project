...
// fromClient is null only for attachments uploaded before uploaderId existed
// (no backfill possible for historical rows) — those render as a neutral
...
// person would.
// memo — same reasoning as MessageBubble: one row in a thread that
// shouldn't re-render for unrelated ticket-metadata refetches. No callback
// props here, so unlike MessageBubble no useCallback wiring is needed on
// the parent side for this memo to actually take effect.
export const AttachmentInlineCard = memo(function AttachmentInlineCard({
  attachment,
  fromClient,
  uploaderName,
}: {
  ...
  return (
    <div className={`flex flex-col ${fromClient ? 'items-start' : 'items-end'}`}>
      <MediaOrChip attachment={attachment} />
      <div className="mt-1 flex items-center gap-1.5 px-1 text-[11px] text-ink-faint">
        <span>
          {uploaderName} · {formatTime(attachment.createdAt, i18n.language)}
        </span>
      </div>
    </div>
  );
