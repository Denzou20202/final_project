import { useState } from 'react';
import { useTranslation } from 'react-i18next';
import { CityModal } from '../components/settings/CityModal.js';
import { useCurrentUser } from '../hooks/useAuth.js';
import { useCities, useDeleteCity } from '../hooks/useCities.js';
import { getErrorMessage } from '../lib/errors.js';
import type { PublicCity } from '../lib/types.js';

export default function CitiesSettingsPage() {
  const { t } = useTranslation();
  const { data: me } = useCurrentUser();
  const isAdmin = me?.role === 'admin';
  const { data: cities, isLoading } = useCities();
  const deleteCity = useDeleteCity();
  const [isCreateOpen, setCreateOpen] = useState(false);
  const [editingCity, setEditingCity] = useState<PublicCity | null>(null);

  function handleDelete(city: PublicCity) {
    if (!window.confirm(t('admin.cities.deleteConfirm', { name: city.name }))) return;
    deleteCity.mutate(city.id);
  }

  const deleteError = deleteCity.error ? getErrorMessage(deleteCity.error) : undefined;

  return (
    <div className="flex h-full flex-col">
      <div className="flex flex-none flex-wrap items-center justify-between gap-2 px-4 pb-3.5 pt-4 sm:px-6">
        <div>
          <div className="font-display text-lg font-bold">{t('admin.cities.title')}</div>
          <div className="mt-0.5 text-[12.5px] text-ink-subtle">{t('admin.cities.subtitle')}</div>
        </div>
        {isAdmin && (
          <button
            type="button"
            onClick={() => setCreateOpen(true)}
            className="rounded-lg bg-brand-600 px-4 py-2 text-[13px] font-semibold text-white hover:bg-brand-hover"
          >
            {t('admin.cities.newCity')}
          </button>
        )}
      </div>

      <div className="flex-1 overflow-auto px-4 pb-6 sm:px-6">
        {isLoading && <div className="py-16 text-center text-sm text-ink-subtle">{t('common.loading')}</div>}

        {deleteError && (
          <p className="mb-3 rounded-lg bg-priority-urgent/10 px-3 py-2 text-sm text-priority-urgent">{deleteError}</p>
        )}

        {!isLoading && (cities ?? []).length === 0 && (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-border bg-surface-card py-16 text-center">
            <div className="font-display text-sm font-semibold text-ink-muted">{t('admin.cities.empty')}</div>
          </div>
        )}

        {!isLoading && (cities?.length ?? 0) > 0 && (
          <div className="overflow-hidden rounded-2xl border border-border bg-surface-card">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border-subtle text-[11px] font-bold uppercase tracking-wide text-ink-faint">
                  <th className="px-4 py-2.5 font-bold">{t('admin.cities.columnName')}</th>
                  {isAdmin && <th className="px-4 py-2.5 font-bold" />}
                </tr>
              </thead>
              <tbody>
                {(cities ?? []).map((city) => (
                  <tr
                    key={city.id}
                    onClick={isAdmin ? () => setEditingCity(city) : undefined}
                    className={`border-b border-border-subtle text-[13.5px] last:border-0 ${isAdmin ? 'cursor-pointer hover:bg-surface-muted' : ''}`}
                  >
                    <td className="px-4 py-3 font-medium">{city.name}</td>
                    {isAdmin && (
                      <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                        <button
                          type="button"
                          onClick={() => setEditingCity(city)}
                          className="text-[12.5px] font-medium text-brand-600 hover:underline"
                        >
                          {t('admin.cities.edit')}
                        </button>
                        <button
                          type="button"
                          onClick={() => handleDelete(city)}
                          className="ml-3 text-[12.5px] font-medium text-priority-urgent hover:underline"
                        >
                          {t('admin.cities.delete')}
                        </button>
                      </td>
                    )}
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </div>

      {isCreateOpen && <CityModal existing={undefined} onClose={() => setCreateOpen(false)} />}
      {editingCity && <CityModal existing={editingCity} onClose={() => setEditingCity(null)} />}
    </div>
  );
}
