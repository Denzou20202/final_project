...
// after an await — same pattern reports.api.ts's CSV export already uses.
// `download` with the real file name is what makes this an actual save-to-
// disk instead of just opening the file in the current tab (which for a
// PDF/image would otherwise just navigate away from the ticket).
