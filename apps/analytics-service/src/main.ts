...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('VeloxDesk — Analytics Service')
    .setDescription('Отчёты: дашборд, среднее время ответа/решения, загрузка по командам, экспорт')
    .setVersion('0.1')
    .addBearerAuth()
    .build();
  ...
  const port = process.env.PORT || process.env.ANALYTICS_SERVICE_PORT || 3005;
