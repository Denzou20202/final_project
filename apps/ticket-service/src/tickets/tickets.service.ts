...
export class TicketsService {
  ...
  async create(dto: CreateTicketDto, actor: JwtPayload): Promise<PublicCreatedTicket> {
    ...
    const { ticket, descriptionComment } = await this.dataSource.transaction(async (manager) => {
      const ticket = await this.ticketsRepository.createWithManager(manager, {
        title: dto.title,
      ...
      // the chat thread.
      const descriptionComment = await manager.save(
        CommentEntity,
  ...
  async merge(id: string, dto: MergeTicketDto, actor: JwtPayload): Promise<PublicTicket> {
    ...
    // Merging tickets from different clients would expose one client's
    // conversation to the other — the target's chat thread would silently
    // absorb the source's comments/attachments, giving the target's client
    // full read access to a stranger's data. Defense in depth: the
    // MergeTicketModal already looks up the target by ticket number (which
    // only returns tickets the actor can see), but a direct API call can
    // still supply any valid UUID as targetTicketId.
    if (source.createdBy !== target.createdBy) {
      throw new BadRequestException(
        'Не можна об\'єднувати тікети різних клієнтів — переписка стане доступна стороннім.',
      );
    }
