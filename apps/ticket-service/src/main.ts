...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('VeloxDesk — Ticket Service')
    .setDescription('Тикеты: CRUD, статусы, приоритеты, назначение, история изменений')
    .setVersion('0.1')
    .addBearerAuth()
    .build();
  ...
  const port = process.env.PORT || process.env.TICKET_SERVICE_PORT || 3011;
